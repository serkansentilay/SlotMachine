...
import { StyleSheet, Easing, View, Animated, ActivityIndicator } from 'react-native'
    ...
    scrollByOffset = (offset, callback) => {
        ...
        Animated.timing(
            this.state.scrollPos,
            {
                ...
                duration: 1750 + (this.props.index * 250),
    ...
    render() {
        return (
            <View style={[styles.reel, { width: this.props.width, height: this.props.height }]}>
                <Animated.View
                    style={{ width: this.props.width, height: this.reelSymbols.length * this.symbolHeight, transform: [{ translateY: this.state.scrollPos }] }}>
